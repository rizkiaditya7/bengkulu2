...
<div class="flex-1 flex flex-col overflow-hidden bg-gray-50 text-gray-900">
    ...
    <main class="flex-1 overflow-x-hidden overflow-y-auto p-6 bg-gray-100">
        <div class="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow">
            <h1 class="text-xl font-semibold text-gray-900 mb-6">Tambah Hero</h1>
            ...
            <form action="{{ route('admin.heroes.store') }}" method="POST" enctype="multipart/form-data"
                ...
                <div>
                    ...
                    <textarea name="subtitle" id="subtitle" rows="3"
                        class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none">{{ old('subtitle') }}</textarea>
                    @error('subtitle')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
