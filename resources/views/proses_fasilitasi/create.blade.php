...
<div class="flex-1 flex flex-col overflow-hidden text-black">
    <header class="bg-white shadow-md p-4 flex justify-between items-center">
        <h2 class="text-2xl font-bold text-gray-800">Tambah Proses Fasilitasi</h2>
    </header>

    <main class="flex-1 overflow-y-auto bg-gray-50 p-6">
        <div class="bg-white p-6 rounded-lg shadow-md max-w-xl mx-auto">
            <form action="{{ route('admin.proses_fasilitasi.store') }}" method="POST">
                @csrf
                <div class="mb-4">
                    ...
                    @error('tahap') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="ikon" value="{{ old('ikon') }}"
                        class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                        placeholder="contoh: fa-envelope, fa-user, fa-briefcase">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="deskripsi" rows="4"
                        class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                        placeholder="Masukkan deskripsi tahap">{{ old('deskripsi') }}</textarea>
